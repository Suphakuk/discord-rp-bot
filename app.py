...
class RegisterModal(discord.ui.Modal, title='ลงทะเบียนเข้าสู่ชมรม'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 1. ตอบกลับนักเรียนให้รอ
        await interaction.response.send_message(f'⏳ ส่งใบสมัครเรียบร้อยแล้วครับ กรุณารอประธานหรือกรรมการตรวจรับรองสักครู่นะครับ', ephemeral=True)
        
        # 2. ส่งใบสมัครไปให้กรรมการกดอนุมัติในห้อง Log
        ...
        if log_channel:
            ...
            # แนบปุ่มอนุมัติ/ปฏิเสธไปด้วย (StaffApprovalView)
...
# เมื่อบอทออนไลน์ ให้มันจำปุ่มต่างๆ เอาไว้ (สำคัญมากสำหรับ Render)
@bot.event
async def on_ready():
