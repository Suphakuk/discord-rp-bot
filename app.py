...
class RegisterModal(discord.ui.Modal, title='ลงทะเบียนเข้าสู่ชมรม'):
    ...
    image_input = discord.ui.TextInput(label='ลิงก์รูปเหรียญที่ให้ตามหา (Image URL)', placeholder='อัปโหลดรูปลงดิสคอร์ดแล้วคัดลอกลิงก์มาวาง', required=True)
    loca_input = discord.ui.TextInput(label='สถานที่ที่เจอเหรียญ', placeholder='ex. โถงบันใดวน', required=True)

    async def on_submit(self, interaction: discord.Interaction):
        new_name = self.name_input.value
        letter_no = self.letter_input.value
        house_name = self.house_input.value.lower()
        location_found = self.loca_input.value
        ...
        if log_channel:
            ...
            embed.set_thumbnail(url=member.display_avatar.url)
            embed.add_field(name="สถานที่เจอเหรียญ", value=location_found, inline=False)
